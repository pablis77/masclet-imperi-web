# Script usando sh en lugar de bash
$EC2_IP = "108.129.139.119"
$PEM_PATH = "C:\Proyectos\primeros proyectos\AWS\masclet-imperi-key.pem"

# Comando para instalar la dependencia usando sh
$installCommand = 'sudo docker exec masclet-frontend-node sh -c "cd /app && npm install es-module-lexer --save"'
$sshInstallCmd = "ssh -i `"$PEM_PATH`" ec2-user@$EC2_IP $installCommand"

Write-Host "Instalando es-module-lexer con sh..." -ForegroundColor Cyan
