...
// Configurar proxy para rutas de autenticación sin prefijo /api/v1
const authProxy = createProxyMiddleware({
  ...
  onError: (err, req, res) => {
    ...
    res.status(500).send('Error al conectar con el backend para autenticación');
  }
});
...
// IMPORTANTE: Configurar rutas API y Auth ANTES del handler SSR
...
app.use(async (req, res, next) => {
  ...
  if (req.url.startsWith('/api/v1') || req.url.startsWith('/auth/')) {
    console.log(`>>> Saltando SSR para ruta API/Auth: ${req.url}`);
    return next();
  }
