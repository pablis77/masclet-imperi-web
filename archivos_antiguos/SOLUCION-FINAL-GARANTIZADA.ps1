# SOLUCION-FINAL-GARANTIZADA.ps1
# Script definitivo para despliegue de Masclet Imperi Frontend

Write-Host "🚀 DESPLIEGUE DEFINITIVO MASCLET IMPERI" -ForegroundColor Cyan
...
$deployScript | Out-File -FilePath "deploy-solution.sh" -Encoding utf8

Write-Host "📂 Preparando servidor remoto..." -ForegroundColor Yellow

# Transferir script de despliegue
Write-Host "📤 Transfiriendo script de despliegue..." -ForegroundColor Yellow
ssh -i "C:\Proyectos\primeros proyectos\AWS\masclet-imperi-key.pem" ec2-user@108.129.139.119 "mkdir -p ~/deploy"
scp -i "C:\Proyectos\primeros proyectos\AWS\masclet-imperi-key.pem" deploy-solution.sh ec2-user@108.129.139.119:~/deploy/

# Transferir archivos de frontend
Write-Host "📤 Transfiriendo archivos de frontend..." -ForegroundColor Yellow
ssh -i "C:\Proyectos\primeros proyectos\AWS\masclet-imperi-key.pem" ec2-user@108.129.139.119 "mkdir -p ~/deploy/dist"
scp -r -i "C:\Proyectos\primeros proyectos\AWS\masclet-imperi-key.pem" frontend/dist/* ec2-user@108.129.139.119:~/deploy/dist/

# Ejecutar script de despliegue
Write-Host "🚀 Ejecutando despliegue en servidor remoto..." -ForegroundColor Green
ssh -i "C:\Proyectos\primeros proyectos\AWS\masclet-imperi-key.pem" ec2-user@108.129.139.119 "cd ~/deploy && chmod +x deploy-solution.sh && sudo ./deploy-solution.sh"
