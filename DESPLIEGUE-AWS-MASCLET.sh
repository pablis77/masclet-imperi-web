#!/bin/bash
# =============================================================================
# DESPLIEGUE COMPLETO DE MASCLET IMPERI WEB EN AWS
# =============================================================================
...
echo "=========================================================="
echo "  DESPLIEGUE COMPLETO DE MASCLET IMPERI FRONTEND EN AWS"
echo "  $(date)"
echo "=========================================================="
...
TEMP_DIR="/tmp/masclet-deploy-$(date +%Y%m%d_%H%M%S)"
mkdir -p $TEMP_DIR
cd $TEMP_DIR
...
echo ">>> Verificando si existe la carpeta dist del frontend..."
if docker exec masclet-api ls -la /app/frontend/dist > /dev/null 2>&1; then
  echo ">>> Copiando la carpeta dist desde el contenedor masclet-api..."
  # Crear un contenedor temporal para copiar los archivos
  docker run --rm -v $TEMP_DIR:/backup --network masclet-network --name temp-copy alpine sh -c "apk add --no-cache rsync && mkdir -p /backup/dist && rsync -av masclet-api:/app/frontend/dist/ /backup/dist/"
else
  echo ">>> No se encontró la carpeta dist en el contenedor masclet-api"
  echo ">>> Descargando la última versión del frontend desde GitHub..."
  
  # Descargar el frontend desde GitHub (ajustar la URL según corresponda)
  if command -v curl &> /dev/null; then
    curl -L -o frontend.zip https://github.com/pablis77/masclet-imperi-web/archive/refs/heads/main.zip
    unzip frontend.zip
    # Buscar la carpeta dist en el repositorio descargado
    DIST_DIR=$(find . -type d -name "dist" | grep -i frontend | head -1)
    if [ -n "$DIST_DIR" ]; then
      cp -r $DIST_DIR ./dist
    else
      echo "ERROR: No se encontró la carpeta dist en el repositorio descargado"
      exit 1
    fi
  else
    echo "ERROR: No se encontró curl para descargar el frontend"
    exit 1
  fi
fi
...
if [ ! -d "dist" ]; then
  echo ">>> La carpeta dist no existe. Creando una estructura mínima..."
...
echo "  $(date)"
echo "=========================================================="
echo ">>> El frontend está disponible en: http://$(hostname -I | awk '{print $1}'):80"
