# Parte 2: Corrección específica del problema de login
# Este script implementa una solución directa para el problema de login

Write-Host "🔐 Iniciando corrección específica del login..." -ForegroundColor Cyan
...
# 3. Crear script para inyectar nuestro arreglo en todas las páginas HTML
echo -e "\n3. Creando script para inyectar nuestro arreglo en las páginas HTML..."
cat > /tmp/inject-login-fix.sh << 'EOL'
#!/bin/bash
# Inyectar script de corrección en páginas HTML de Nginx

# Buscar en el directorio principal y subdirectorios
find /usr/share/nginx/html -type f -name "*.html" | while read file; do
  echo "Modificando $file..."
  # Comprobar si ya contiene nuestro script
  if ! grep -q "login-direct-fix.js" "$file"; then
    # Inyectar nuestro script antes del cierre de head
    sed -i 's|</head>|<script src="/login-direct-fix.js"></script></head>|' "$file"
  fi
done
EOL

# 4. Ejecutar script de inyección
echo -e "\n4. Ejecutando script de inyección..."
...
Get-Content -Path $tempFile.FullName -Raw | 
    ...
    ssh -i $KEY_PATH $SERVER "cat > /tmp/parte2_corregir_login.sh"

Remove-Item -Path $tempFile.FullName

Write-Host "🔒 Ejecutando corrección específica del login..." -ForegroundColor Cyan
ssh -i $KEY_PATH $SERVER "chmod +x /tmp/parte2_corregir_login.sh && sh /tmp/parte2_corregir_login.sh"

Write-Host @"
✅ Corrección del login aplicada correctamente.
...
Por favor, prueba el login con:
- Usuario: admin
- Contraseña: admin123

También comprueba que el logout funciona correctamente.
