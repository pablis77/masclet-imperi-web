...
Usa la configuración correcta de login que ya sabemos que funciona
"""
...
def obtener_token():
    """Obtiene token de autenticación usando configuración que sabemos que funciona"""
    color_print(f"=== Obteniendo token para {USERNAME} ===", "blue")
    
    # URL que sabemos que funciona según diagnóstico
...
def main():
    ...
    probar_endpoint("/dashboard/stats/", token)
    
    # Probar explotaciones
    color_print("\n== Endpoints de Explotaciones ==", "blue")
    explotaciones_ok = probar_endpoint("/dashboard/explotacions/", token)
    
    if explotaciones_ok:
        # Obtener la primera explotación para probar detalles
        ...
        if response.status_code == 200:
            ...
            if explotaciones:
                ...
                color_print(f"\n== Probando explotación específica (ID: {exp_id}) ==", "blue")
                probar_endpoint(f"/dashboard/explotacions/{exp_id}/", token)
    
    # Probar otros endpoints importantes
    color_print("\n== Otros endpoints críticos ==", "blue")
    probar_endpoint("/dashboard/summary/", token)
    probar_endpoint("/dashboard/partos/", token)
    probar_endpoint("/dashboard/recent-activity/", token)
    
    # Conclusión
    color_print("\n=== VERIFICACIÓN COMPLETADA ===", "blue")
    color_print("Backend y base de datos están funcionando correctamente.", "green")
