...
const SimpleAnimalTable: React.FC<AnimalTableProps> = ({ initialFilters = {}, id, canEdit = false }) => {
  // Estados para los datos
  const [animals, setAnimals] = useState<Animal[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  // Función simple para cargar todos los animales
  const loadAllAnimals = async () => {
    try {
      ...
      const response = await animalService.getAnimals({ all: true });
      
      console.log(`Cargados ${response.items?.length || 0} animales`);
  ...
  // Renderizar icono según tipo de animal
  const getAnimalIcon = (animal: Animal) => {
    if (animal.genere === 'M') {
      return <span className="text-2xl">🐂</span>; // Toro
    } else {
      if (animal.alletar !== '0') {
        return <span className="text-2xl">🐄</span>; // Vaca amamantando
      } else {
        return <span className="text-2xl">🐮</span>; // Vaca
      }
    }
  };

  // Renderizar insignia de estado
...
export default SimpleAnimalTable;
